...
class QtCreatorKitSetup:
    ...
    def _get_cmake_config_vars(self) -> list[str]:
        ...
        config.append(f"CMAKE_PREFIX_PATH:PATH=%{{Qt:QT_INSTALL_PREFIX}};{install}")
        config.append(f"CMAKE_MODULE_PATH:PATH={install}/lib64/cmake;{install}/lib/cmake")
        config.append("CMAKE_EXPORT_COMPILE_COMMANDS:BOOL=ON")
        config.append("CMAKE_BUILD_TYPE:STRING=RelWithDebInfo")
