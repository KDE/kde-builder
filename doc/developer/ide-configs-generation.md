...
For Qt Creator, `kde-builder` generates a `CMakeUserPresets.json` tailored for Qt Creator, alongside the existing `.qtcreator` helper files.

Starting with Qt Creator 20, the generated preset and its `vendor.qt.io/QtCreator/1.0` metadata have everything needed to get started in seconds.
Run `kde-builder --setup-qtcreator-kde-kit` once, open the top `CMakeLists.txt` file, and enjoy hacking!

Older versions will need some manual tweaks, described below.

### Kit

This step is needed once so Qt Creator has the shared `org.kde.builder` kit available.
Qt Creator 20+ picks it up automatically from the generated preset, while older versions may still require selecting it manually.
...
`kde-builder --setup-qtcreator-kde-kit` helps you to easily create a Kit for Qt Creator. You need to run this command once.
`kde-builder` uses [SDK Tool](https://doc.qt.io/qtcreator/creator-sdk-tool.html) to register the new kit. By default, it searches for sdktool in `~/Qt/Tools/sdktool/libexec/qtcreator`, `/usr/libexec/qtcreator/sdktool` and `/usr/lib/qtcreator/sdktool`.
If you have installed it in a different place, make sure you specify its path by setting `qtcreator-sdktool-path` property in `~/.config/kde-builder.yaml`. `qtcreator-settings-path` property from the same file can be used to override the default (`~/.config/QtProject/qtcreator`) Qt Creator settings folder.

The new kit contains everything needed to enable painless KDE development.
It sets the build and run environment variables, common CMake options, toolchain, debugger, and Qt version.
After this kit is registered, please check if we picked the right Qt version for you.
