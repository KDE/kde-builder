...
def main():
    ...
    with open(f"{KB_PACKAGE_DIR}/resources/kde-builder-logging.yaml", "r") as f:
    ...
    if setup_steps:
        ...
        exit(fr.setup_user_system(setup_steps))

    from kde_builder.application import Application  # noqa: E402
    from kde_builder.kb_exception import KBException  # noqa: E402
    ...
    try:
        ...
        unrecognized_name = err.unknown_project_name
        ctx = app.context
        ...
        if unrecognized_name.startswith("-"):
            ...
            msg = dedent(f"""
                 r[*] y[{unrecognized_name}] is not found in KDE projects, and there is no defined third-party project with such name in the config.
                 r[*] Note that kde-builder only recognizes KDE projects that are marked in repo-metadata as "software" kind.
                 r[*] Such kinds as "website", "infrastructure", "packaging", "other" are ignored.
                """, preserve_len=1)
