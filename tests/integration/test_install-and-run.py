...
def test_install_and_run():
    ...
    #     assert os.path.islink(f"{tempInstallDir}/bin/kde-builder"), "kdesrc-build is supposed to be a symlink"
    #     assert os.unlink(f"{tempInstallDir}/bin/kde-builder") is None, "Remove kdesrc-build symlink, so it will not conflict with install"
        ...
        #     os.environ["PATH"] = f"{tempInstallDir}/share/kdesrc-build:" + os.environ.get("PATH")  # Currently, we install to share.
