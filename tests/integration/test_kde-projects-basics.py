...
def test_kde_projects():
    ...
    assert moduleList[0].get_option("tag") == "tag-setmod2", "options block works for indirect reference to kde-projects module"
    assert moduleList[0].get_option("cmake-generator") == "Ninja", "Global opts seen even with other options"
    assert moduleList[1].get_option("cmake-generator") == "Make", "options block works for kde-projects module-set"
    assert moduleList[1].get_option("cmake-options") == "-DSET_FOO:BOOL=ON", "module options block can override set options block"
    assert moduleList[2].get_option("cmake-generator") == 'Make', "options block works for kde-projects module-set after options"
    assert moduleList[2].get_option("cmake-options") == "-DSET_FOO:BOOL=ON", "module-set after options can override options block"
